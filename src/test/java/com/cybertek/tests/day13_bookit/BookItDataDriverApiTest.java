...
public class BookItDataDriverApiTest {

    public static List<Map<String,String>> getAllUserCredentials(){
        ExcelUtil excelUtil = new ExcelUtil("BookItQa3.xlsx","QA3"); //open excelfile/sheet
    ...
    public void allUsersLoginTest(Map<String,String> userInfo){
        given().accept(ContentType.JSON).and().queryParams(userInfo).when().get("/sign").then().assertThat().statusCode(200).and().body("accessToken",not(emptyString())).log().all();
